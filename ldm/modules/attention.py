...
class CrossAttention_hard(nn.Module):
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0.):
        super().__init__()
        inner_dim = dim_head * heads
        context_dim = default(context_dim, query_dim)
    ...
    def forward(self, x, context=None, mask=None):
        ...
        atten_max, atten_max_arg = torch.max(sim, dim=-1)
        
        out = self.bis(v, 1, atten_max_arg)
        out *= atten_max.unsqueeze(2)
...
class BasicTransformerBlockV8_refV5(nn.Module):
    # weight init method: 1
    ATTENTION_MODES = {
        'softmax': CrossAttention, # Vanilla attention
        'softmax-xformers': MemoryEfficientCrossAttention
    }
    ...
    def __init__(self, dim, n_heads, d_head, dropout=0., context_dim=None, gated_ff=True, checkpoint=True,
                 disable_self_attn=False, use_lr=True):
        super().__init__()
        attn_mode = "softmax-xformers" if XFORMERS_IS_AVAILBLE else "softmax"
        assert attn_mode in self.ATTENTION_MODES
        attn_cls = self.ATTENTION_MODES[attn_mode]
        ...
        self.attn3_lr_lna = attn_cls(query_dim=dim, context_dim=None,
                                     heads=n_heads, dim_head=d_head, dropout=dropout) # is self-attn if context is none
    ...
    def _forward(self, x, context=None, lr=None, ref=None, h=None, w=None, gen_mode=False):
        if not gen_mode:
            ...
            cond = rearrange(self.zero_conv_r_lna(cond), 'b c h w -> b (h w) c').contiguous()
            x = x + cond
            cond = self.attn3_lr_lna(self.norm4_lna(x), lr)
            cond = rearrange(cond, 'b (h w) c -> b c h w', h=h, w=w).contiguous()
            cond = rearrange(self.zero_conv_lna(cond), 'b c h w -> b (h w) c').contiguous()
            x = x + cond
        x = self.attn1(self.norm1(x), context=context if self.disable_self_attn else None) + x
        x = self.attn2(self.norm2(x), context=context) + x
        x = self.ff(self.norm3(x)) + x
...
class SpatialTransformerV2d4(nn.Module):
    """
    Transformer block for image-like data.
    First, project the input (aka embedding)
    and reshape to b, t, d.
...
class SpatialTransformerV8_refV5(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head,
                 depth=1, dropout=0., context_dim=None,
                 disable_self_attn=False, use_linear=False,
                 use_checkpoint=False, use_lr=True, merge_x2=True, dubch=False):
        super().__init__()
        if exists(context_dim) and not isinstance(context_dim, list):
            context_dim = [context_dim]
        self.in_channels = in_channels
        ...
        if use_lr:
            if self.dubch:
                if self.merge_x2:
                    self.dubconv_lna = nn.Conv2d(in_channels//2, in_channels, kernel_size=3, stride=2, padding=1)
                else:
                    self.dubconv_lna = nn.Conv2d(in_channels//2, in_channels, kernel_size=3, stride=1, padding=1)
            else:
                if self.merge_x2:
                    self.avgpool_clr = nn.AvgPool2d(2)

            self.norm_clr_lna = Normalize(in_channels)
            if not use_linear:
                self.proj_in_clr_lna = nn.Conv2d(in_channels, inner_dim, kernel_size=1, stride=1, padding=0)
            else:
                self.proj_in_clr_lna = nn.Linear(in_channels, inner_dim)

        if self.dubch:
            if self.merge_x2:
                self.dubconv_r_lna = nn.Conv2d(in_channels//2, in_channels, kernel_size=3, stride=2, padding=1)
            else:
                self.dubconv_r_lna = nn.Conv2d(in_channels//2, in_channels, kernel_size=3, stride=1, padding=1)
        else:
            if self.merge_x2:
                self.avgpool_cref = nn.AvgPool2d(2)
        self.norm_cref_lna = Normalize(in_channels)

        if not use_linear:
            self.proj_in_cref_lna = nn.Conv2d(in_channels, inner_dim,
                                        kernel_size=1, stride=1, padding=0)
        else:
            self.proj_in_cref_lna = nn.Linear(in_channels, inner_dim)

        self.transformer_blocks = nn.ModuleList([
            BasicTransformerBlockV8_refV5(inner_dim, n_heads, d_head, dropout=dropout,
                                          context_dim=context_dim[d], disable_self_attn=disable_self_attn,
                                          checkpoint=use_checkpoint, use_lr=use_lr)
            for d in range(depth)
        ])
