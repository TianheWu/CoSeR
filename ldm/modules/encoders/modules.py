...
class FrozenOpenCLIPImageTokenEmbedder(AbstractEncoder):
    """
    Uses the OpenCLIP transformer encoder for text
    """
    LAYERS = [
    ...
    def __init__(self, arch="ViT-H-14", version="laion2b_s32b_b79k", device="cuda", max_length=77,
                 freeze=True, layer="last", output_tokens=True, global_average_pool=True):
        super().__init__()
        assert layer in self.LAYERS
        model, _, vis_preprocess = open_clip.create_model_and_transforms(arch, device=torch.device('cpu'), pretrained=version)
        # del model.visual
        self.model = model
        self.vis_preprocess = vis_preprocess
        ...
        self.model.visual.global_average_pool = global_average_pool
    ...
    def encode_with_vision_transformer(self, img):
        _, tokens = self.model.visual(img)
