...
class UNetModelAiA(nn.Module):
    ...
    """
    TRANSFORMER_MODES = {
        "V8_refV5": SpatialTransformerV8_refV5,
    }
    ...
    def __init__(
        ...
        self.model_channels = model_channels
        self.out_channels = out_channels
        spatialtransformer = self.TRANSFORMER_MODES[transformer_version]
        ...
        self.middle_block = TimestepEmbedSequential(
            ...
            AttentionBlock(
                ...
                use_new_attention_order=use_new_attention_order,
                            ...
                            use_checkpoint=use_checkpoint, use_lr=True, merge_x2=merge_x2
                        ),
            ResBlock(
                ch,
                time_embed_dim,
    ...
    def forward(self, x, timesteps=None, condition=None, y=None, gen_mode=False,**kwargs):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
        ...
        if not gen_mode:
            ...
            lr_condition_dec = lr_condition

            ref_condition = condition['ref_struct']
...
class ControlNet(nn.Module):
    def __init__(
        ...
        # )
        self.hint_zero_conv = zero_module(conv_nd(dims, in_channels, model_channels, 3, padding=1))
        ...
        self.middle_block = TimestepEmbedSequential(
            ...
            AttentionBlock(
                ...
                num_head_channels=dim_head,
                use_new_attention_order=use_new_attention_order,
            ) if not use_spatial_transformer else SpatialTransformerV2d4(  # always uses a self-attn
                ch, num_heads, dim_head, depth=transformer_depth, context_dim=context_dim,
                disable_self_attn=disable_middle_self_attn, use_linear=use_linear_in_transformer,
                use_checkpoint=use_checkpoint
            ),
    ...
    def forward(self, x, hint, timesteps, context, lr_prompt=None, **kwargs):
        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False)
        emb = self.time_embed(t_emb)

        guided_hint = self.hint_zero_conv(hint)

        outs = []

        h = x.type(self.dtype)
        if guided_hint.shape != h.shape:
            gen_num = guided_hint.shape[3] // h.shape[3]
            h = torch.cat([h for _ in range(gen_num)], 3)
        
        for module, zero_conv in zip(self.input_blocks, self.zero_convs):
            if guided_hint is not None:
                h = module(h, emb, context, lr_prompt=lr_prompt)
                h += guided_hint
                guided_hint = None
            else:
                h = module(h, emb, context, lr_prompt=lr_prompt)
            outs.append(zero_conv(h, emb, context, lr_prompt=lr_prompt))
