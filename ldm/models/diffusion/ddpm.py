...
class LatentDiffusion(DDPM):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        x = x.to(self.device)
        encoder_posterior = self.encode_first_stage(x)
        z = self.get_first_stage_encoding(encoder_posterior).type(self.datatype).detach()
...
class LatentDiffusionCogSR(DDPM):
    """main class"""
    def __init__(self,
                 first_stage_config,
                 cond_stage_config,
                 ...
                 use_atten_init=True,
                 drop_rate=0.0,
                 pretrained_presr_qformer_path=None,
                 num_timesteps_cond=None,
                 cond_stage_key="image",
                 cond_stage_trainable=False,
                 concat_mode=True,
        ...
        self.datatype = torch.float16 if use_fp16 else torch.float32
        self.fixed_cond = fixed_cond
        ...
        self.pre_sr_model = instantiate_from_config(pre_sr_config)
        self.global_adapter = instantiate_from_config(cog_enc_config)
        self.cond_stage_forward = cond_stage_forward
        self.clip_denoised = False
        self.bbox_tokenizer = None
        ...
        if ckpt_path is not None:
            self.init_from_ckpt(ckpt_path, ignore_keys)
            self.init_control_weight(self.structcond_stage_model, ckpt_path)
            self.restarted_from_ckpt = True

        self.use_atten_init = use_atten_init
        if self.use_atten_init:
            self.init_atten_weight()
        ...
        self.pretrained_presr_qformer_path = pretrained_presr_qformer_path
        if self.pretrained_presr_qformer_path is not None:
            for name, param in self.pre_sr_model.named_parameters():
                param.requires_grad = False
            for name, param in self.global_adapter.named_parameters():
                param.requires_grad = False
            self.global_adapter_optimize = False
            self.pre_sr_optimize = False

            self.pre_sr_model.eval()
            self.global_adapter.eval()

            self.init_pq_weight(self.pretrained_presr_qformer_path)
        else:
            self.global_adapter_optimize = True
            self.pre_sr_optimize = True
        ...
        param_list = []
        print('>>>>>>>>>>>>>>>>pre_sr_model>>>>>>>>>>>>>>>>>>>>')
        for name, params in self.pre_sr_model.named_parameters():
            if params.requires_grad:
                param_list.append(name)
        print(param_list)
        param_list = []
        print('>>>>>>>>>>>>>>>>global_adapter>>>>>>>>>>>>>>>>>>>>')
        for name, params in self.global_adapter.named_parameters():
            if params.requires_grad:
                param_list.append(name)
        print(param_list)
    ...
    def init_pq_weight(self, pretrained_presr_qformer_path):
        pq_weight = torch.load(pretrained_presr_qformer_path, map_location='cpu')["state_dict"]
        m, u = self.load_state_dict(pq_weight, strict=False)
        if len(m) > 0:
            m_temp = []
            for item in m:
                if ('pre_sr_model' in item) or ('global_adapter' in item):
                    m_temp.append(item)
            print('Missing keys (in pre_sr_model and global_adapter):')
            print(m_temp)
        if len(u) > 0:
            print('Unexpected keys (in pre_sr_model and global_adapter):')
            print(u)
    
    def init_control_weight(self, model, weight_path):
        load_net = torch.load(weight_path, map_location='cpu')['state_dict']
        new_dict = {}
        for param_name, _ in model.named_parameters():
            if not (('zero_convs' in param_name) or ('middle_block_out' in param_name) or ('hint_zero_conv' in param_name) or ('lna_kv' in param_name)):
                new_dict[param_name] = load_net['model.diffusion_model.'+param_name]

        m, n = model.load_state_dict(new_dict, strict=False)
        print('Copy weight of controlnet from diffusion unet:')
        if len(m) > 0:
            print(f"Missing keys: {m}")
        if len(n) > 0:
            print(f"Unexpected keys: {n}")
    
    def init_atten_weight(self):
        params = dict(self.model.named_parameters())
        for name, param in params.items():
            if 'attn3_lr' in name:
                param.data.copy_(params[name.replace("attn3", "attn1").replace("_lr", "").replace("_lna", "")].data)
            if '_clr' in name:
                param.data.copy_(params[name.replace("_clr", "").replace("_lna", "")].data)
            if 'norm4' in name:
                param.data.copy_(params[name.replace("norm4", "norm1").replace("_lna", "")].data)
            if 'attn4_ref' in name:
                param.data.copy_(params[name.replace("attn4", "attn1").replace("_ref", "").replace("_lna", "")].data)
            if '_cref' in name:
                param.data.copy_(params[name.replace("_cref", "").replace("_lna", "")].data)
        print('Attention initialization complete!')
    ...
    @torch.no_grad()
    def validation_step(self, batch, batch_idx):
        _, loss_dict_no_ema = self.shared_step_val(batch)
        with self.ema_scope():
            _, loss_dict_ema = self.shared_step_val(batch)
            loss_dict_ema = {key + '_ema': loss_dict_ema[key] for key in loss_dict_ema}
        self.log_dict(loss_dict_no_ema, prog_bar=False, logger=True, on_step=False, on_epoch=True)
        self.log_dict(loss_dict_ema, prog_bar=False, logger=True, on_step=False, on_epoch=True)

    def shared_step_val(self, batch, **kwargs):
        x, c, gt, ref = self.get_input_val(batch, self.first_stage_key)
        loss = self.forward_val(x, c, gt, ref)
        return loss
    ...
    def get_input(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
        ...
        self.lq_clip = self.lq.detach().clone().contiguous()
    ...
    @torch.no_grad()
    def get_input_val(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
                  cond_key=None, return_original_cond=False, bs=None, val=False, text_cond=[''], return_gt=False, resize_lq=True):
        ...
        # self.lq = self.lq.contiguous()  # for the warning: grad and param do not obey the gradient layout contract
        im_lq = im_lq*2 - 1.0
        self.gt_val = im_gt*2 - 1.0
        self.ref_val = im_ref*2 - 1.0

        im_lq = torch.clamp(im_lq, -1.0, 1.0)
        self.ref_val = torch.clamp(self.ref_val, -1.0, 1.0)
    ...
    def shared_step(self, batch, **kwargs):
        x, c, gt, ref = self.get_input(batch, self.first_stage_key)
        loss = self(x, c, gt, ref)
        return loss
    ...
    def forward_val(self, x, c, gt, ref, *args, **kwargs):
        index = np.random.randint(0, self.num_timesteps, size=x.size(0))
        t = torch.from_numpy(index)
        t = t.to(self.device).long()
    ...
    def p_losses(self, x_start, condition_dic, t, t_ori, z_gt, pre_sr_loss, clip_adapt_loss, noise=None):
        ...
        if self.pre_sr_optimize:
            loss += (1.0 * pre_sr_loss)
            loss_dict.update({f'{prefix}/pre_sr_loss': pre_sr_loss})
        
        if self.global_adapter_optimize:
            loss += (1.0 * clip_adapt_loss)
            loss_dict.update({f'{prefix}/clip_adapt_loss': clip_adapt_loss})

        return loss, loss_dict

    def p_mean_variance(self, x, c, c_ne, cfg, t, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
                        return_x0=False, score_corrector=None, corrector_kwargs=None, t_replace=None, gen_mode=False):
        ...
        if cfg==1:
            model_out = self.apply_model(x, t_in, c, return_ids=return_codebook_ids, gen_mode=gen_mode)
        else:
            model_out = self.apply_model(x, t_in, c, return_ids=return_codebook_ids, gen_mode=gen_mode)
            model_out_uncond = self.apply_model(x, t_in, c_ne, return_ids=return_codebook_ids, gen_mode=gen_mode)
            model_out = model_out_uncond + cfg * (model_out - model_out_uncond)
    ...
    def p_mean_variance_canvas(self, x, c, c_ne, cfg, t, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
        ...
        for row in range(grid_rows):
            ...
            for col in range(grid_cols):
                ...
                cond_tile = c['lr'][:, :, input_start_y:input_end_y, input_start_x:input_end_x]
                cond_list.append(cond_tile)
                cond_tile_r = c['reference'][:, :, input_start_y:input_end_y, input_start_x:input_end_x]
                cond_list_r.append(cond_tile_r)

                if len(input_list) == batch_size or col == grid_cols-1:
                    input_list = torch.cat(input_list, dim=0)
                    cond_list = torch.cat(cond_list, dim=0)
                    cond_list_r = torch.cat(cond_list_r, dim=0)

                    struct_c = self.structcond_stage_model(input_list, cond_list, t_in[:input_list.size(0)], c['prompt_emb'], lr_prompt=c['lr_prompt_emb'])
                    reference_c = self.structcond_stage_model(input_list, cond_list_r, t_in[:input_list.size(0)], c['prompt_emb'], lr_prompt=c['lr_prompt_emb'])
                    c['struct'] = struct_c
                    c_ne['struct'] = copy.deepcopy(struct_c)
                    c['ref_struct'] = reference_c
                    c_ne['ref_struct'] = copy.deepcopy(reference_c)

                    if cfg==1:
                        model_out = self.apply_model(input_list, t_in[:input_list.size(0)], c, return_ids=return_codebook_ids)
                    else:
                        model_out = self.apply_model(input_list, t_in[:input_list.size(0)], c, return_ids=return_codebook_ids)
                        model_out_uncond = self.apply_model(input_list, t_in[:input_list.size(0)], c_ne, return_ids=return_codebook_ids)
                        model_out = model_out_uncond + cfg * (model_out - model_out_uncond)
    ...
    @torch.no_grad()
    def p_sample(self, x, c, c_ne, cfg, t, clip_denoised=False, repeat_noise=False,
                 return_codebook_ids=False, quantize_denoised=False, return_x0=False,
                 temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None, t_replace=None, gen_mode=False):
        b, *_, device = *x.shape, x.device
        outputs = self.p_mean_variance(x=x, c=c, c_ne=c_ne, cfg=cfg, t=t, clip_denoised=clip_denoised,
                                       return_codebook_ids=return_codebook_ids,
                                       quantize_denoised=quantize_denoised,
                                       return_x0=return_x0,
                                       score_corrector=score_corrector, corrector_kwargs=corrector_kwargs, t_replace=t_replace, gen_mode=gen_mode)
        if return_codebook_ids:
            raise DeprecationWarning("Support dropped.")
            model_mean, _, model_log_variance, logits = outputs
        elif return_x0:
    ...
    @torch.no_grad()
    def p_sample_canvas(self, x, c, c_ne, cfg, t, clip_denoised=False, repeat_noise=False,
                 return_codebook_ids=False, quantize_denoised=False, return_x0=False,
        ...
        b, *_, device = *x.shape, x.device
        outputs = self.p_mean_variance_canvas(x=x, c=c, c_ne=c_ne, cfg=cfg, t=t, clip_denoised=clip_denoised,
                                       return_codebook_ids=return_codebook_ids,
                                       quantize_denoised=quantize_denoised,
                                       return_x0=return_x0,
    ...
    @torch.no_grad()
    def p_sample_loop(self, cond, cond_ne, cfg, shape, return_intermediates=False,
                      x_T=None, verbose=True, callback=None, timesteps=None, quantize_denoised=False,
                      mask=None, x0=None, img_callback=None, start_T=None,
                      log_every_t=None, time_replace=None, adain_fea=None, interfea_path=None, gen_mode=False):
        ...
        for i in iterator:
            ...
            if not gen_mode:
                if t_replace is None:
                    struct_c = self.structcond_stage_model(img, cond['lr'], ts, cond['prompt_emb'], lr_prompt=cond['lr_prompt_emb'])
                    reference_c = self.structcond_stage_model(img, cond['reference'], ts, cond['prompt_emb'], lr_prompt=cond['lr_prompt_emb'])
                else:
                    struct_c = self.structcond_stage_model(img, cond['lr'], t_replace, cond['prompt_emb'], lr_prompt=cond['lr_prompt_emb'])
                    reference_c = self.structcond_stage_model(img, cond['reference'], t_replace, cond['prompt_emb'], lr_prompt=cond['lr_prompt_emb'])
            
                cond['struct'] = struct_c
                cond_ne['struct'] = copy.deepcopy(struct_c)
                cond['ref_struct'] = reference_c
                cond_ne['ref_struct'] = copy.deepcopy(reference_c)
            
            # if interfea_path is not None:
            #     batch_list.append(struct_cond_input)

            img = self.p_sample(img, cond, cond_ne, cfg, ts,
                                clip_denoised=self.clip_denoised,
                                quantize_denoised=quantize_denoised, t_replace=t_replace, gen_mode=gen_mode)
    ...
    @torch.no_grad()
    def p_sample_loop_canvas(self, cond, cond_ne, cfg, shape, return_intermediates=False,
                      x_T=None, verbose=True, callback=None, timesteps=None, quantize_denoised=False,
                      mask=None, x0=None, img_callback=None, start_T=None,
        ...
        for i in iterator:
            ...
            img = self.p_sample_canvas(img, cond, cond_ne, cfg, ts,
                                clip_denoised=self.clip_denoised,
    ...
    @torch.no_grad()
    def sample(self, cond, cond_ne, cfg, batch_size=16, return_intermediates=False, x_T=None,
               verbose=True, timesteps=None, quantize_denoised=False,
               mask=None, x0=None, shape=None, time_replace=None, adain_fea=None, interfea_path=None, start_T=None, gen_mode=False, **kwargs):
        ...
        #     else:
        #         cond = [c[:batch_size] for c in cond] if isinstance(cond, list) else cond[:batch_size]
        last, inter = self.p_sample_loop(cond, cond_ne, cfg,
                                  shape,
                                  return_intermediates=return_intermediates, x_T=x_T,
                                  verbose=verbose, timesteps=timesteps, quantize_denoised=quantize_denoised,
                                  mask=mask, x0=x0, time_replace=time_replace, adain_fea=adain_fea, interfea_path=interfea_path, start_T=start_T, gen_mode=gen_mode)
        return last, inter

    @torch.no_grad()
    def sample_tile(self, cond, cond_ne, cfg, batch_size=16, return_intermediates=False, x_T=None,
               verbose=True, timesteps=None, quantize_denoised=False,
        ...
        last, inter = self.p_sample_loop_canvas(cond, cond_ne, cfg,
                                  shape,
                                  return_intermediates=return_intermediates, x_T=x_T,
                                  verbose=verbose, timesteps=timesteps, quantize_denoised=quantize_denoised,
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
                   ...
                   plot_diffusion_rows=False, split='train', **kwargs):

        use_ddim = ddim_steps is not None

        log = dict()
        if split == 'train':
            z, c_lq, z_gt, z_ref, x, gt, yrec, xc = self.get_input(batch, self.first_stage_key,
                                            return_first_stage_outputs=True,
                                            force_c_encode=True,
                                            return_original_cond=True,
                                            bs=N, val=True)
        else:
            z, c_lq, z_gt, z_ref, x, gt, yrec, xc = self.get_input_val(batch, self.first_stage_key,
                                            return_first_stage_outputs=True,
                                            force_c_encode=True,
                                            return_original_cond=True,
                                            bs=N, val=True)
        ...
        if split == 'train':
            log["reference"] = self.ref
        else:
            log["reference"] = self.ref_val

        c, _ = self.cond_stage_model(['' for _ in range(N)])

        if split == 'train':
            image_c = self.pre_sr_model(self.lq_clip)
        else:
            image_c = self.pre_sr_model(self.lq_clip_val)
        log['pre_sr'] = image_c*2.0 - 1.0

        image_features = self.cond_stage_model.encode_with_vision_transformer(self.clip_transfrom(image_c))

        cog_embed = self.global_adapter(image_features)

        condition_dic = {'prompt_emb': c, 'lr_prompt_emb': cog_embed, 'lr': z, 'reference': z_ref}

        # text_ne = ['worst quality, normal quality, low quality, low res, blurry, text, watermark, logo, banner, extra digits, cropped, jpeg artifacts, signature, username, error, sketch ,duplicate, ugly, monochrome, horror, geometry, mutation, disgusting']

        # c_ne, _ = self.cond_stage_model(text_ne)

        # condition_dic_ne = {'prompt_emb': None, 'lr_prompt_emb': c_ne, 'lr': z, 'reference': z_ref}
        condition_dic_ne = {}
        ...
        if sample:
            ...
            samples, z_denoise_row = self.sample(cond=condition_dic, cond_ne=condition_dic_ne, cfg=cfg, batch_size=N, timesteps=cur_time_step, return_intermediates=True, time_replace=self.time_replace)
            x_samples = self.decode_first_stage(samples)
            log["samples"] = x_samples
            if plot_denoise_rows:
                denoise_grid = self._get_denoise_row_from_list(z_denoise_row)
    ...
    def configure_optimizers(self):
        ...
        if self.pre_sr_optimize:
            params = params + list(self.pre_sr_model.parameters())
        if self.global_adapter_optimize:
            params = params + list(self.global_adapter.parameters())
        if self.learn_logvar:
...
class LatentDiffusionCogAdapter(pl.LightningModule):
    """main class"""
    def __init__(self,
                 cond_stage_config,
                 pre_sr_config,
    ...
    def get_input(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
        ...
        out = [self.lq_clip, self.gt, text_cond]
    ...
    def get_input_val(self, batch, k=None, return_first_stage_outputs=False, force_c_encode=False,
        ...
        out = [self.lq_clip_val, self.gt_val, text_cond]

        return out


    def training_step(self, batch, batch_idx):
        x, gt, c = self.get_input(batch)
        loss, loss_dict = self(x, gt, c)
        self.log_dict(loss_dict, prog_bar=True,
                      logger=True, on_step=True, on_epoch=True)
    ...
    @torch.no_grad()
    def validation_step(self, batch, batch_idx):
        x, gt, c = self.get_input_val(batch)
        loss, loss_dict = self(x, gt, c)
        self.log_dict(loss_dict, prog_bar=True,
                      logger=True, on_step=False, on_epoch=True)

    def forward(self, x, gt, c, *args, **kwargs):
        c, tokens = self.cond_stage_model(c)

        # clip image embedding
        image_c = self.pre_sr_model(x)
        gt_resize = F.interpolate(gt, size=(image_c.size(-2), image_c.size(-1)), mode='bicubic')
        pre_sr_loss = (gt_resize - image_c).abs().mean()
        image_features = self.cond_stage_model.encode_with_vision_transformer(self.clip_transfrom(image_c))
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, condition, gen_mode=False):
        out = self.diffusion_model(x, t, condition, gen_mode=gen_mode)
