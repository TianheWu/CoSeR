	...
	with torch.no_grad():
		with precision_scope("cuda"):
			with model.ema_scope():
				for name in names_list:
					# load image
					image = trans(Image.open(os.path.join(opt.inputdir, name))).unsqueeze(0).cuda()
					...
					if im_lq_bs.shape[2] > opt.vqgantile_size or im_lq_bs.shape[3] > opt.vqgantile_size:
						...
						for im_lq_pch, index_infos in im_spliter:
							...
							if model.fixed_cond:
								init_latent = model.get_first_stage_encoding(model.encode_first_stage(im_lq_pch).mode())
							else:
